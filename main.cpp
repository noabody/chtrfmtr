...
private:
    ...
    std::optional<ParseResult> invokeSystemParser(const QString &systemName, const QString &rawLine) const {
        ...
        for (const auto &p : systemCodePatterns.value(systemName)) {
            ...
            if (m.hasMatch()) return ParseResult{m.captured(0).toUpper().trimmed(), p.key, static_cast<int>(m.capturedLength(0))};
        }
        return std::nullopt;
    }
    ...
    void importMyBoyChtEngine(const QString &path) {
        ...
        if (!doc.setContent(&f, true, &err, &line, &col)) {
            throw std::runtime_error(
                QString("Failed parsing MyBoy XML target: %1 (line %2, column %3)")
                    .arg(err)
                    .arg(line)
                    .arg(col)
                    .toStdString()
    ...
    void importKronosYctEngine(const QString &path) {
        ...
        for (int i = 0; i < total; ++i) {
            ...
            int nameLength = qMax(1, int(quint8(b[pos++])) - 1);
            
            if (pos + nameLength + 5 > b.size()) {
                break;
            }
    ...
    void registerModules() {
        ...
        auto nesParser = [this](const QString& input, const QString&) {
            ...
            auto res = invokeSystemParser("NES", input);
            ...
            if (m.hasMatch() &&
                m.captured(1).toInt(nullptr, 16) >= 0x8000) {
        
                QString gg = invokeGameGenieEncodeNES(res->code);
        ...
        auto retroParser = [this](const QString& input, const QString& target) {
            if (target.isEmpty()) {
                return ParseOutput{};
            }
        
            QString system = systemKey(target);
        
            if (!systemCodePatterns.contains(system)) {
                return ParseOutput{};
            }
            ...
            for (const auto& p : systemCodePatterns.value(system)) {
                auto it = p.regex.globalMatch(sanitized);
        
                while (it.hasNext()) {
                    auto m = it.next();
                    QString c = m.captured(0).toUpper().trimmed();
        
                    if (!c.isEmpty()) {
                        results << c;
                        match += static_cast<int>(m.capturedLength(0));
                    }
                }
            }
